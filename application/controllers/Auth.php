...
class Auth extends Public_Controller
{
    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            log_message('error', "Attempting login for identity: " . $identity);

            if ($this->ion_auth->login($identity, $password, $remember)) {
                log_message('error', "Login successful for identity: " . $identity);
                ...
                if ($result['role'] != 1) {
                    redirect(site_url('/courses/my_courses'), 'refresh');
                } else {
                    redirect(site_url('/admin'), 'refresh');
                }

            } else {
                log_message('error', "Login failed for identity: " . $identity . ". Errors: " . $this->ion_auth->errors());
    ...
    {
        // Single Device Login: clear session ID on logout
        $user_id = $this->session->userdata('user_id');
        if ($user_id) {
