...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE)
	{
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt') {
			$bcrypt_check = $this->bcrypt->verify($password, $hash_password_db->password);
			log_message('error', "Ion Auth hash_password_db: Bcrypt check result: " . ($bcrypt_check ? 'TRUE' : 'FALSE'));
			if ($bcrypt_check) {
				return TRUE;
			}
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$this->trigger_events('extra_where');
		log_message('error', "Ion Auth Login: Searching for user with identity: " . $identity . " (Identity Column: " . $this->identity_column . ")");
		...
		if ($this->is_max_login_attempts_exceeded($identity)) {
			log_message('error', "Ion Auth Login: Max login attempts exceeded for: " . $identity);
		...
		if ($query->num_rows() === 1) {
			$user = $query->row();
			log_message('error', "Ion Auth Login: User found: " . $user->email . " (ID: " . $user->id . ")");

			$password_check = $this->hash_password_db($user->id, $password);

			if ($password_check === TRUE) {
				log_message('error', "Ion Auth Login: Password check SUCCESS");
				if ($user->active == 0) {
					log_message('error', "Ion Auth Login: User is NOT active");
					$this->trigger_events('post_login_unsuccessful');
			...
			log_message('error', "Ion Auth Login: Password check FAILED for user ID: " . $user->id);
		} else {
			log_message('error', "Ion Auth Login: User NOT found for identity: " . $identity . ". Query row count: " . $query->num_rows());
		}
