...
class Events_model extends CI_Model
{
    ...
    {
        return $this->db->query("SELECT e.id, e.title, e.start_date, e.status, 'admin/events/view/' url, 'event' type, (SELECT COUNT(ebm.id) FROM e_bookings_members ebm WHERE ebm.e_bookings_id IN (SELECT eb.id FROM e_bookings eb WHERE eb.events_id = e.id)) total_bookings FROM events e ORDER BY total_bookings DESC LIMIT 5")
            ->result();
    }
