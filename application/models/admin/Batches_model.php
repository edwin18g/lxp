...
class Batches_model extends CI_Model
{
    ...
    {
        return $this->db->query("SELECT b.id, b.title, b.start_date, b.fees, 'admin/batches/view/' url, 'batch' type, (SELECT COUNT(bbm.id) FROM b_bookings_members bbm WHERE bbm.b_bookings_id IN (SELECT bb.id FROM b_bookings bb WHERE bb.batches_id = b.id)) total_bookings FROM batches b ORDER BY total_bookings DESC LIMIT 5")
            ->result();
    }
